...
for iey = 1:length(eps_y)
    for iex = 1:length(eps_x)
        ...
        MSE_all = MSEvalues_all(d_X(iex), d_XY, MSEmox_mean, MSEmox_kisl_mean, ...
        ...
        %blankRow = NaN(1, size(rowData, 2));
        %writematrix(blankRow, filename, 'WriteMode', 'append');
...
% add two more blank rows at the end of all (eps_x, eps_y) combinations
writecell(repmat({''}, 1, size(rowData,2)), filename, 'WriteMode', 'append');
writecell(repmat({''}, 1, size(rowData,2)), filename, 'WriteMode', 'append');
writecell(repmat({''}, 1, size(rowData,2)), filename, 'WriteMode', 'append');
writecell(repmat({''}, 1, size(rowData,2)), filename, 'WriteMode', 'append');
